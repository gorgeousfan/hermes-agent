---
title: "Claude Code — 委托编码给 Claude Code CLI（功能、PR）"
sidebar_label: "Claude Code"
description: "委托编码给 Claude Code CLI（功能、PR）"
---
...
通过 Hermes 终端将编码任务委托给 [Claude Code](https://code.claude.com/docs/en/cli-reference)（Anthropic 的自主编码代理 CLI）。Claude Code v2.x 可以读取文件、编写代码、运行 shell 命令、生成子代理并自主管理 git 工作流。

## 前置条件

- **安装：** `npm install -g @anthropic-ai/claude-code`
- **认证：** 运行 `claude` 一次以登录（Pro/Max 的浏览器 OAuth，或设置 `ANTHROPIC_API_KEY`）
- **控制台认证：** `claude auth login --console` 用于 API 密钥计费
- **SSO 认证：** `claude auth login --sso` 用于企业版
- **检查状态：** `claude auth status`（JSON）或 `claude auth status --text`（可读）
...
Hermes 与 Claude Code 的交互有两种根本不同的方式。根据任务选择。

### 模式 1：打印模式（`-p`）— 非交互式（大多数任务的首选）

打印模式运行一次性任务，返回结果并退出。无需 PTY，无交互式提示。这是最简洁的集成路径。
...
**打印模式跳过所有交互式对话框** — 无工作区信任提示，无权限确认。这使其非常适合自动化。

### 模式 2：通过 tmux 的交互式 PTY — 多轮会话

交互模式提供一个完整的对话式 REPL，你可以发送后续提示、使用斜杠命令并实时观看 Claude 工作。**需要 tmux 编排。**
...
- 需要人在环路中决策的任务
- 探索性编码会话
- 需要使用 Claude 斜杠命令（`/compact`、`/review`、`/model`）

## PTY 对话框处理（交互模式的关键）

Claude Code 在首次启动时会呈现最多两个确认对话框。你**必须**通过 tmux send-keys 处理这些：
...
**注意：** 首次接受目录信任后，信任对话框不会再次出现。只有权限对话框在每次使用 `--dangerously-skip-permissions` 时重现。
...
| `cat file \| claude -p "query"` | 以管道内容为标准输入上下文 |
| `claude -c` | 继续此目录最近的对话 |
| `claude -r "id"` | 通过 ID 或名称恢复特定会话 |
| `claude auth login` | 登录（加 `--console` 用于 API 计费，`--sso` 用于企业版） |
| `claude auth status` | 检查登录状态（返回 JSON；`--text` 可读） |
...
**关键字段：** `session_id` 用于恢复，`num_turns` 用于代理循环计数，`total_cost_usd` 用于支出追踪，`subtype` 用于成功/错误检测（`success`、`error_max_turns`、`error_budget`）。

### 流式 JSON 输出
对于实时令牌流，使用带 `--verbose` 的 `stream-json`：
```
terminal(command="claude -p 'Write a summary' --output-format stream-json --verbose --include-partial-messages", timeout=60)
```

返回换行符分隔的 JSON 事件。用 jq 过滤实时文本：
```
...
从 JSON 结果中解析 `structured_output`。Claude 在返回前根据 schema 验证输出。
...
| 加载内容 | 标志 |
|---------|------|
| 系统提示附加 | `--append-system-prompt "text"` 或 `--append-system-prompt-file path` |
...
| `-r, --resume <id>` | 通过 ID 或名称恢复特定会话（无 ID 则交互式选择） |
| `--fork-session` | 恢复时创建新会话 ID 而不是复用原来的 |
| `--session-id <uuid>` | 为对话使用特定 UUID |
| `--no-session-persistence` | 不保存会话到磁盘（仅打印模式） |
| `--add-dir <paths...>` | 授予 Claude 访问额外工作目录 |
| `-w, --worktree [name]` | 在 `.claude/worktrees/<name>` 的隔离 git 工作树中运行 |
| `--tmux` | 为工作树创建 tmux 会话（需要 `--worktree`） |
| `--ide` | 启动时自动连接到有效的 IDE |
| `--chrome` / `--no-chrome` | 启用/禁用 Chrome 浏览器集成用于 web 测试 |
...
| `--max-turns <n>` | 限制代理循环（仅打印模式；防止失控） |
| `--max-budget-usd <n>` | 以美元设置 API 支出上限（仅打印模式） |
| `--fallback-model <model>` | 默认模型过载时自动切换（仅打印模式） |
...
| `--allow-dangerously-skip-permissions` | 启用绕过作为*选项*而不默认启用 |
...
| `--json-schema <schema>` | 强制结构化 JSON 输出匹配 schema |
| `--verbose` | 完整逐轮输出 |
...
| 标志 | 效果 |
|------|--------|
| `-d, --debug [filter]` | 启用调试日志，带可选类别过滤器（如 `"api,hooks"`、`"!1p,!file"`） |
...
| `--brief` | 启用用于代理到用户通信的 `SendUserMessage` 工具 |
...
| `/compact [focus]` | 压缩上下文以节省令牌；CLAUDE.md 在压缩后存活。如 `/compact focus on auth logic` |
| `/clear` | 清除对话历史以重新开始 |
| `/context` | 可视化上下文使用情况为彩色网格，带优化提示 |
| `/cost` | 查看令牌使用量，带每模型和缓存命中分解 |
| `/resume` | 切换到或恢复不同的会话 |
| `/rewind` | 回退到对话或代码中的之前检查点 |
| `/btw <question>` | 提问旁白而不增加上下文成本 |
...
| `/plan [description]` | 进入带任务规划自动启动的规划模式 |
| `/loop [interval]` | 在会话中安排重复任务 |
| `/batch` | 自动为大型并行更改创建工作树（5-30 个工作树） |

### 配置和工具
| 命令 | 用途 |
|---------|---------|
| `/model [model]` | 中途切换模型（使用方向键调整努力程度） |
| `/effort [level]` | 设置推理努力：`low`、`medium`、`high`、`max` 或 `auto` |
| `/init` | 为项目记忆创建 CLAUDE.md 文件 |
| `/memory` | 打开 CLAUDE.md 进行编辑 |
| `/config` | 打开交互式设置配置 |
| `/permissions` | 查看/更新工具权限 |
| `/agents` | 管理专业子代理 |
| `/mcp` | 用于管理 MCP 服务器的交互式 UI |
| `/add-dir` | 添加额外工作目录（对 monorepo 有用） |
| `/usage` | 显示计划限制和速率限制状态 |
...
用法：`/deploy production` — `$ARGUMENTS` 被用户的输入替换。

### 技能（自然语言调用）
与斜杠命令（手动调用）不同，`.claude/skills/` 中的技能是 Claude 在任务匹配时通过自然语言自动调用的 markdown 指南：
...
| `@` | 使用自动完成引用文件/目录（如 `@./src/api/`） |
| `#` | 快速添加到 CLAUDE.md 记忆（如 `# Use 2-space indentation`） |
| `/` | 斜杠命令 |

### 专业提示："ultrathink"
在提示中使用关键词"ultrathink"以在特定轮次获得最大推理努力。这会触发最深的思考模式，无论当前的 `/effort` 设置如何。
...
```
在 `.claude/worktrees/feature-x` 创建隔离的 git 工作树并为其创建 tmux 会话。在可用时使用 iTerm2 原生窗格；添加 `--tmux=classic` 用于传统 tmux。
...
**要具体。** 不用"写好代码"，而是用"JS 使用 2 空格缩进"或"测试文件命名为 `.test.ts` 后缀"。具体指令节省纠错轮次。

### 规则目录（模块化 CLAUDE.md）
对于有很多规则的项目，使用规则目录而不是一个庞大的 CLAUDE.md：
- **项目规则：** `.claude/rules/*.md` — 团队共享，git 跟踪
- **用户规则：** `~/.claude/rules/*.md` — 个人，全局

规则目录中的每个 `.md` 文件作为额外上下文加载。这比把所有内容塞进一个 CLAUDE.md 更整洁。

### 自动记忆
Claude 自动将学到的项目上下文存储在 `~/.claude/projects/<project>/memory/` 中。
- **限制：** 每个项目 25KB 或 200 行
- 这与 CLAUDE.md 分开 — 它是 Claude 关于项目的自己的笔记，跨会话积累

## 自定义子代理

在 `.claude/agents/`（项目）、`~/.claude/agents/`（个人）或通过 `--agents` CLI 标志（会话）中定义专业代理：
...
| 钩子 | 触发时机 | 常见用途 |
|------|--------------|------------|
| `UserPromptSubmit` | Claude 处理用户提示前 | 输入验证、日志记录 |
| `PreToolUse` | 工具执行前 | 安全门控、阻止危险命令（exit 2 = 阻止） |
| `PostToolUse` | 工具完成后 | 自动格式化代码、运行 linter |
| `Notification` | 权限请求或输入等待时 | 桌面通知、警报 |
| `Stop` | Claude 完成响应时 | 完成日志记录、状态更新 |
| `SubagentStop` | 子代理完成时 | 代理编排 |
| `PreCompact` | 上下文记忆清除前 | 备份会话转录 |
...
| `-s local` | 此项目（个人） | `.claude/settings.local.json`（gitignored） |
...
```
`--strict-mcp-config` 忽略除 `--mcp-config` 中的所有 MCP 服务器。

在聊天中引用 MCP 资源：`@github:issue://123`

### MCP 限制和调优
- **工具描述：** 工具描述和服务器指令每个服务器 2KB 上限
- **结果大小：** 默认有上限；使用 `maxResultSizeChars` 注解允许最多 **500K** 字符用于大输出
- **输出令牌：** `export MAX_MCP_OUTPUT_TOKENS=50000` — 限制 MCP 服务器的输出以防止上下文溢出
- **传输：** `stdio`（本地进程）、`http`（远程）、`sse`（服务器发送事件）
...
| 变量 | 效果 |
|----------|--------|
| `ANTHROPIC_API_KEY` | 认证的 API 密钥（OAuth 的替代方案） |
| `CLAUDE_CODE_EFFORT_LEVEL` | 默认努力：`low`、`medium`、`high`、`max` 或 `auto` |
| `MAX_THINKING_TOKENS` | 限制思考令牌（设为 `0` 完全禁用思考） |
| `MAX_MCP_OUTPUT_TOKENS` | 限制 MCP 服务器的输出（默认值各不相同；如设为 `50000`） |
| `CLAUDE_CODE_NO_FLICKER=1` | 启用交替屏幕渲染以消除终端闪烁 |
| `CLAUDE_CODE_SUBPROCESS_ENV_SCRUB` | 从子进程中删除凭据以提高安全性 |

## 成本和性能提示

1. **在打印模式中使用 `--max-turns`** 以防止失控循环。大多数任务从 5-10 开始。
2. **使用 `--max-budget-usd`** 设置成本上限。注意：系统提示缓存创建最低约 $0.05。
3. **使用 `--effort low`** 用于简单任务（更快更便宜）。对于复杂推理使用 `high` 或 `max`。
4. **使用 `--bare`** 用于 CI/脚本以跳过插件/钩子发现开销。
5. **使用 `--allowedTools`** 限制仅使用所需工具（如仅 `Read` 用于审查）。
6. **在交互会话中使用 `/compact`** 当上下文变大时。
7. **管道输入** 而不是让 Claude 读取文件，当你只需要分析已知内容时。
8. **使用 `--model haiku`** 用于简单任务（更便宜），`--model opus` 用于复杂多步骤工作。
9. **在打印模式中使用 `--fallback-model haiku`** 以优雅处理模型过载。
10. **为不同任务开始新会话** — 会话持续 5 小时；新鲜上下文更高效。
11. **在 CI 中使用 `--no-session-persistence`** 以避免在磁盘上积累保存的会话。

## 陷阱和注意事项

1. **交互模式需要 tmux** — Claude Code 是完整的 TUI 应用。在 Hermes 终端中单独使用 `pty=true` 可以工作，但 tmux 提供 `capture-pane` 用于监控和 `send-keys` 用于输入，这对编排至关重要。
2. **`--dangerously-skip-permissions` 对话框默认为"No, exit"** — 必须发送向下然后 Enter 才能接受。打印模式（`-p`）完全跳过这个。
3. **`--max-budget-usd` 最低约 $0.05** — 仅系统提示缓存创建就需要这么多。设置更低会立即报错。
4. **`--max-turns` 仅适用于打印模式** — 在交互会话中被忽略。
5. **Claude 可能使用 `python` 而不是 `python3`** — 在没有 `python` 符号链接的系统上，Claude 的 bash 命令第一次会失败，但它会自我纠正。
6. **会话恢复需要相同目录** — `--continue` 找到当前工作目录的最近会话。
7. **`--json-schema` 需要足够的 `--max-turns`** — Claude 在生成结构化输出之前必须读取文件，这需要多轮。
8. **信任对话框每个目录仅出现一次** — 仅第一次，然后缓存。
9. **后台 tmux 会话持久存在** — 完成后总是使用 `tmux kill-session -t <name>` 清理。
10. **斜杠命令（如 `/commit`）仅在交互模式中有效** — 在 `-p` 模式中，用自然语言描述任务。
11. **`--bare` 跳过 OAuth** — 需要 `ANTHROPIC_API_KEY` 环境变量或设置中的 `apiKeyHelper`。
12. **上下文退化是真实存在的** — 在上下文窗口使用率超过 70% 时，AI 输出质量明显下降。使用 `/context` 监控并主动 `/compact`。

## Hermes 代理规则

1. **单任务首选打印模式（`-p`）** — 更简洁，无对话框处理，结构化输出
...
6. **查找 `❯` 提示符** — 表示 Claude 在等待输入（完成或在提问）
7. **清理 tmux 会话** — 完成后终止它们以避免资源泄漏
8. **向用户报告结果** — 完成后总结 Claude 做了什么和发生了什么变化
9. **不要终止慢会话** — Claude 可能在做多步骤工作；先检查进度
10. **使用 `--allowedTools`** — 限制任务实际需要的能力
