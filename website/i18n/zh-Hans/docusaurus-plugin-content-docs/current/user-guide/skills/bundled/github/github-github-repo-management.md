---
title: "GitHub 仓库管理 — 克隆/创建/派生仓库；管理远程仓库、发布版本"
sidebar_label: "GitHub 仓库管理"
description: "克隆/创建/派生仓库；管理远程仓库、发布版本"
---
...
创建、克隆、派生、配置和管理 GitHub 仓库。每个部分先展示 `gh` 方法，然后是 `git` + `curl` 回退方式。
...
```bash
# 通过 HTTPS 克隆（配合凭证助手或嵌入令牌的 URL 使用）
...
注意：对于密钥操作，`gh secret set` 简单得多。如果需要设置密钥且 `gh` 不可用，建议仅为此操作安装 gh。
