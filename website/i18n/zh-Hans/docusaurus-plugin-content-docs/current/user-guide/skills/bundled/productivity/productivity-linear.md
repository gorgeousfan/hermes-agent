---
title: "Linear — Linear：通过 GraphQL + curl 管理事项、项目和团队"
sidebar_label: "Linear"
description: "Linear：通过 GraphQL + curl 管理事项、项目和团队"
---
...
Linear：通过 GraphQL + curl 管理事项、项目和团队。
...
| 作者 | Hermes Agent |
| 许可证 | MIT |
| 标签 | `Linear`, `项目管理`, `事项`, `GraphQL`, `API`, `生产力` |
...
直接通过 GraphQL API 使用 `curl` 管理 Linear 事项、项目和团队。无需 MCP 服务器、无需 OAuth 流程、无需额外依赖。
...
- **认证头：** `Authorization: $LINEAR_API_KEY`（API 密钥无 "Bearer" 前缀）
- **所有请求为 POST** 并设置 `Content-Type: application/json`
...
对于不需要手写 GraphQL 的更快单行命令，此技能附带基于标准库的 Python CLI，位于 `scripts/linear_api.py`。零依赖。相同的认证（读取 `LINEAR_API_KEY`）。
...
python3 "$SCRIPT" get-document 38359beef67c      # 通过 URL 的 slugId 获取文档
...
使用脚本的场景：你想快速获得答案而不需要手写 GraphQL。使用 curl 的场景：你需要脚本未封装的查询，或想内联组合过滤器。
...
| `triage` | 需要审查的传入事项 |
| `backlog` | 已确认但未计划 |
| `unstarted` | 已计划/就绪但未开始 |
| `started` | 正在积极处理 |
| `completed` | 已完成 |
| `canceled` | 不会处理 |

每个团队有自己的命名状态（如"进行中"是 `started` 类型）。要更改事项的状态，你需要目标状态的 `stateId`（UUID）— 先查询工作流状态。
...
curl -s -X POST https://api.linear.app/graphql \
  ...
  -d '{
    ...
    "variables": {
      "input": {
        "name": "Q2 认证大改造",
        "description": "用 OAuth2 和 PKCE 替换旧认证",
        "teamIds": ["TEAM_UUID"]
      }
    }
  }' | python3 -m json.tool
...
Linear **文档** 是与事项一起存储的散文式文档（RFC、规范、笔记）。它们有自己的 `documents` 根查询和 `document(id:)` 单次获取。
...
尾部十六进制段是 `slugId`。示例：`https://linear.app/nousresearch/document/rfc-hermes-permission-gateway-discord-38359beef67c` → `slugId` 是 `38359beef67c`。

**重要架构细节：** Markdown 正文在 `content` 字段中。ProseMirror JSON 在 `contentState` 中（不是 `contentData` — 该字段不存在且 API 返回 400）。

### 通过 slugId 获取文档

`document(id:)` 仅接受 UUID。要通过 URL 的十六进制 slug 获取，过滤集合：
...
Linear 的架构没有 `searchDocuments` 根查询。改用标题子字符串过滤：
...
3. **列出或搜索事项** 找到需要处理的内容
4. **创建事项** 带团队 ID、标题、描述、优先级
5. **更新状态** 通过设置 `stateId` 到目标工作流状态
6. **添加评论** 跟踪进度
7. **标记完成** 通过设置 `stateId` 到团队的"completed"类型状态

## 速率限制

- 每个 API 密钥每小时 5,000 个请求
- 每小时 3,000,000 复杂度点
- 使用 `first: N` 限制结果并降低复杂度消耗
...
- 如果创建事项时省略 `stateId`，Linear 默认使用第一个待办状态
