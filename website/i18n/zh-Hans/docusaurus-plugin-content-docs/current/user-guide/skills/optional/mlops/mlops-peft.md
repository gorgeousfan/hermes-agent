...
使用 LoRA、QLoRA 及 25+ 种方法对 LLM 进行参数高效微调。适用于在有限 GPU 内存下微调大模型（7B-70B）、以 <1% 参数训练且精度损失极小、或多适配器服务的场景。HuggingFace 官方库，与 transformers 生态深度集成。
...
- 训练ing small models (&lt;1B parameters)
...
```python
from transformers import AutoModelForCausalLM, AutoTokenizer, 训练ingArguments, 训练er
...
| **8** | ~7M | Low | Good | **推荐 starting point** |
...
| 前缀调优 | 0.1% | Low | Medium | Generation control |
...
```python
from trl import SFT训练er, SFTConfig
...
| Method | GPU Memory | 训练able Params |
|--------|-----------|------------------|
...
训练ingArguments(num_train_epochs=5)

# Lower learning rate
训练ingArguments(learning_rate=1e-4)
```
...
5. **保存 adapters frequently** (small files, easy rollback)
