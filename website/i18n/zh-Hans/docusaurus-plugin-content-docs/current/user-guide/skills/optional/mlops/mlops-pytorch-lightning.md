...
description: "高级 PyTorch 框架，包含 训练er 类、自动分布式训练（DDP/FSDP/DeepSpeed）、回调系统和最少的样板代码"
---
...
高级 PyTorch 框架，包含 Trainer 类、自动分布式训练（DDP/FSDP/DeepSpeed）、回调系统和最少的样板代码。同套代码可从笔记本扩展到超级计算机。适用于追求简洁训练循环且内置最佳实践的场景。
...
| Tags | `PyTorch Lightning`, `训练ing Framework`, `Distributed 训练ing`, `DDP`, `FSDP`, `DeepSpeed`, `High-Level API`, `Callbacks`, `Best Practices`, `Scalable` |
...
# PyTorch Lightning - High-Level 训练ing Framework
...
**将 PyTorch 转换为 Lightning** (3 steps):
...
# 步骤 1：定义 LightningModule（组织 PyTorch 代码）
...
# Step 3: 训练 with 训练er (handles everything else!)
trainer = L.训练er(max_epochs=10, accelerator='gpu', devices=2)
model = LitModel()
trainer.fit(model, train_loader)
```

**That's it!** 训练er handles:
...
### Workflow 1: 来自 PyTorch to Lightning
...
# 训练
trainer = L.训练er(max_epochs=10, accelerator='gpu')
...
- 多节点 support (just set `num_nodes=2`)
...
- **测试ed**: 1M+ downloads/month, battle-tested
...
```python
# 测试 on CPU first
trainer = L.训练er(accelerator='cpu', devices=1)

# Then GPU
trainer = L.训练er(accelerator='gpu', devices=1)
```
...
**Distributed strategies**: See [references/distributed.md](https://github.com/NousResearch/hermes-agent/blob/main/optional-skills/mlops/pytorch-lightning/references/distributed.md) for DDP, FSDP, DeepSpeed ZeRO 集成, multi-node setup.
...
- **单 GPU**: Works
- **多 GPU**: DDP (default), FSDP, or DeepSpeed
