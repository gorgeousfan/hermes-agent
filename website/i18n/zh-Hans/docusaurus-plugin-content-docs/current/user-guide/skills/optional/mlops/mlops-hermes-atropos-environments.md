---
title: "Hermes Atropos Environments — 构建、测试和调试 Hermes Agent RL 环境，用于 Atropos 训练"
sidebar_label: "Hermes Atropos Environments"
description: "构建、测试和调试 Hermes Agent RL 环境，用于 Atropos 训练"
---
...
构建、测试和调试 Hermes Agent RL 环境，用于 Atropos 训练. 涵盖 HermesAgentBaseEnv 接口、奖励函数、代理循环集成、带工具的评估、wandb 日志记录和三种 CLI 模式（serve/process/evaluate）。适用于在 hermes-agent 仓库中创建、审查或修复 RL 环境。
...
11. **Always ask the user for their inference setup** — Never hardcode or assume a specific provider/model. See the "Inference 设置" section above.
...
2. **Ask the user for inference setup** (see "Inference 设置" section above)
