...
使用 FastMCP 在 Python 中构建、测试、检查、安装和部署 MCP 服务器。适用于创建新的 MCP 服务器、将 API 或数据库封装为 MCP 工具、暴露资源或提示词，或为 Claude Code、Cursor 或 HTTP 部署准备 FastMCP 服务器的场景。
...
### 5. 测试 the Server Before Integrating It Anywhere
