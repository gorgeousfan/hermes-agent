...
description: "使用 torchtitan 进行 PyTorch 原生分布式 LLM 预训练，支持 4D 并行（FSDP2、TP、PP、CP）"
---
...
使用 torchtitan 进行 PyTorch 原生分布式 LLM 预训练，支持 4D 并行（FSDP2、TP、PP、CP）. 适用于大规模预训练 Llama 3.1、DeepSeek V3 或自定义模型，从 8 到 512+ GPU，支持 Float8、torch.compile 和分布式检查点。
...
| Tags | `Model Architecture`, `Distributed 训练ing`, `TorchTitan`, `FSDP2`, `Tensor Parallel`, `Pipeline Parallel`, `Context Parallel`, `Float8`, `Llama`, `Pretraining` |
...
# 来自 source (latest features, requires PyTorch nightly)
...
### Workflow 2: 多节点训练 with SLURM

```
Multi-Node 训练ing:
- [ ] Step 1: 配置 parallelism for scale
...
训练ing auto-resumes if checkpoint exists in configured folder.
...
- [ ] Step 2: 配置 4D parallelism
- [ ] Step 3: 启动 on 512 GPUs
```
...
- 训练ing on H100s with Float8 support
...
**Adding custom models**: See [references/custom-models.md](https://github.com/NousResearch/hermes-agent/blob/main/optional-skills/mlops/torchtitan/references/custom-models.md) for 训练Spec protocol.
