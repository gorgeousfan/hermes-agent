...
输入 `@` 后跟一个引用，即可将内容直接注入到消息中。Hermes 会内联展开引用，并在 `--- Attached Context ---` 部分下追加内容。
...
| `@file:path/to/file.py:10-25` | 注入指定行范围（1-indexed，含首尾） |
| `@folder:path/to/dir` | 注入目录树列表及文件元数据 |
| `@diff` | 注入 `git diff`（未暂存的 working tree 变更） |
...
- 单独的 `@` 后跟部分文本，显示当前目录下匹配的文件和文件夹

## 行范围

`@file:` 引用支持行范围，可精确注入内容：

```text
@file:src/main.py:42        # 第 42 行
@file:src/main.py:10-25     # 第 10 至 25 行（含）
```
...
上下文引用有边界限制，防止压垮模型的上下文窗口：

| 阈值 | 值 | 行为 |
|-----------|-------|----------|
| 软限制 | 上下文长度的 25% | 追加警告，展开继续 |
| 硬限制 | 上下文长度的 50% | 拒绝展开，原始消息原样返回 |
| 文件夹条目 | 最多 200 个文件 | 超出的条目替换为 `- ...` |
...
通过 MIME 类型和空字节扫描检测二进制文件。已知的文本扩展名（`.py`、`.md`、`.json`、`.yaml`、`.toml`、`.js`、`.ts` 等）会绕过 MIME 检测。二进制文件会附带警告被拒绝。

## 平台可用性

上下文引用主要是 **CLI 功能**。在交互式 CLI 中可用，`@` 触发 Tab 补全，引用在消息发送到 agent 之前展开。

在 **消息平台**（Telegram、Discord 等）中，`@` 语法不会被网关展开——消息按原样传递。Agent 本身仍可通过 `read_file`、`search_files` 和 `web_extract` 工具引用文件。

## 与上下文压缩的交互

当对话上下文被压缩时，展开的引用内容会包含在压缩摘要中。这意味着：

- 通过 `@file:` 注入的大型文件内容会计入上下文使用量
- 如果对话稍后被压缩，文件内容会被摘要化（不会逐字保留）
...
| Git 命令失败 | 警告并附上 git stderr |
| URL 无内容 | 警告："no content extracted" |
