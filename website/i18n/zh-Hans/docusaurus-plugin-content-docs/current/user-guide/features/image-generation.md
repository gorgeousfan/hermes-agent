...
description: 通过 FAL.ai 文生图；支持 9 个模型，含 FLUX 2、GPT Image（1.5 与 2）、Nano Banana Pro、Ideogram、Recraft V4 Pro 等，可用 hermes tools 切换。
sidebar_label: 文生图
sidebar_position: 6
---
...
Hermes Agent 通过 FAL.ai 根据文字提示生成图像。默认内置 9 个模型，在速度、画质与成本上各有取舍。当前模型可通过 `hermes tools` 配置，并持久化在 `config.yaml`。
...
| `fal-ai/gpt-image-2` | ~20s | 顶级文字渲染 + 中日韩文字，具备世界知识的写实能力 | $0.04–0.06/张 |
...
`fal-ai/gpt-image-1.5` 与 `fal-ai/gpt-image-2` 的请求画质固定为 `medium`（约 1024×1024 下 $0.034–$0.06/张）。面向用户**不开放** `low` / `high` 档位，以便 Nous Portal 侧计费在全体用户间更可预期（档位价差约 3–22×）。若需要更便宜的选项，请选 Klein 9B 或 Z-Image Turbo；若追求更高画质，可考虑 Nano Banana Pro 或 Recraft V4 Pro。
...
GPT Image 2 映射到 4:3 预设而不是 16:9，是因为它要求至少 655,360 像素；`landscape_16_9` 预设（1024×576 = 589,824）会被拒绝。
