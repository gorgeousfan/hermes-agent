...
description: "开箱即用的自动化模板 — 定时任务、GitHub 事件触发器、API webhook 和多技能工作流"
---

# 自动化模板

常用自动化模式的复制粘贴模板。每个模板使用 Hermes 的内置 [cron 调度器](/docs/user-guide/features/cron) 进行基于时间的触发，以及 [webhook 平台](/docs/user-guide/messaging/webhooks) 进行事件驱动的触发。
...
| 触发器 | 如何 | 工具 |
|---------|-----|------|
| **Schedule** | 按节奏运行（每小时、每晚、每周） | `cronjob` 工具或 `/cron` 斜杠命令 |
| **GitHub Event** | 在 PR 打开、推送、issue、CI 结果时触发 | Webhook 平台 (`hermes webhook subscribe`) |
| **API Call** | 外部服务 POST JSON 到你的端点 | Webhook 平台 (config.yaml 路由或 `hermes webhook subscribe`) |

所有三种都支持交付到 Telegram、Discord、Slack、SMS、电子邮件、GitHub 评论或本地文件。
:::
...
每晚标记、优先处理和汇总新 issue。向你的团队频道发送摘要。

**触发器：** Schedule (nightly)
...
每次部署后触发冒烟测试。你的 CI/CD 流水线在部署完成时 POST 到 webhook。
...
将监控警报与近期变更关联，起草响应。适用于 Datadog、PagerDuty、Grafana 或任何可以 POST JSON 的警报系统。
...
每 30 分钟检查一次端点。仅在有服务下线时通知。

**触发器：** Schedule (every 30 min)
...
| Telegram | `--deliver telegram` | 主页频道，或 `telegram:CHAT_ID` 指定特定频道 |
| Discord | `--deliver discord` | 主页频道，或 `discord:CHANNEL_ID` |
| Slack | `--deliver slack` | 主页频道 |
...
| `{__raw__}` | 完整 JSON 负载（截断到 4000 字符） |
| `{sender.login}` | 触发事件的 GitHub 用户 |

### [SILENT] 模式

当 cron 任务的响应包含 `[SILENT]` 时，交付被抑制。使用此方法可以避免在安静运行时收到通知垃圾信息：

```
If nothing noteworthy happened, respond with [SILENT].
```

这意味着你只在 agent 有内容报告时收到通知。
