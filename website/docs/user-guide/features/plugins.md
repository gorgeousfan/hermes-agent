...
| An **STT backend** (custom whisper binary, local ASR CLI) | Config-driven — set `HERMES_LOCAL_STT_COMMAND` env var to a shell template. OR Python backend plugin — `ctx.register_transcription_provider()` for non-built-in engines (OpenRouter, SenseAudio, Gemini-STT, etc.). | [Voice Message Transcription (STT)](/docs/user-guide/features/tts#voice-message-transcription-stt) · [Python plugin guide](/docs/user-guide/features/tts#python-plugin-providers-stt) |
