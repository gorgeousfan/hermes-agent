...
# Module-level constants for backward compatibility (imports from cron.__init__
# and direct attribute access).  Internal code should prefer the _resolve_*
# helpers below so that profile-aware get_hermes_home() is re-evaluated at
# call time instead of being frozen at import time (#25295).
...
def _resolve_cron_dir() -> Path:
    """Resolve cron directory dynamically so profile changes are honored."""
    return get_hermes_home().resolve() / "cron"


def _resolve_jobs_file() -> Path:
    """Resolve jobs.json path dynamically so profile changes are honored."""
    return _resolve_cron_dir() / "jobs.json"


def _resolve_output_dir() -> Path:
    """Resolve cron output directory dynamically so profile changes are honored."""
    return _resolve_cron_dir() / "output"
...
def remove_job(job_id: str) -> bool:
    ...
    if len(jobs) < original_len:
        ...
        job_output_dir = _resolve_output_dir() / canonical_id
...
def save_job_output(job_id: str, output: str):
    ...
    ensure_dirs()
    job_output_dir = _resolve_output_dir() / job_id
