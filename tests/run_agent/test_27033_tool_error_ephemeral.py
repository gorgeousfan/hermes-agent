...
4. ``gateway/run.py`` ``_looks_like_stale_tool_error`` correctly identifies stale errors
...
def test_gateway_stale_tool_error_helper():
    """The gateway's _looks_like_stale_tool_error identifies all error patterns."""
    from gateway.run import _looks_like_stale_tool_error

    # Identifies by _is_error flag
    assert _looks_like_stale_tool_error(
