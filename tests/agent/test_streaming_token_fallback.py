...
Re-port of #12740 onto current main.  The streaming code path was
extracted from ``run_agent.py`` into ``agent/chat_completion_helpers.py``
upstream, so the helper now lives there.
"""

from types import SimpleNamespace

from agent.chat_completion_helpers import apply_streaming_token_fallback


class TestStreamingTokenEstimation:
    def test_fallback_estimates_when_usage_none(self):
        """When ``usage_obj`` is ``None`` the helper must estimate both token fields."""
    ...
    def test_preserves_valid_zero_usage(self):
        ...
        ``prompt_tokens=0`` value (common with cache hits on MiniMax/Kimi)
        is NOT overwritten by the estimation path.
        """
        api_messages = [{"role": "user", "content": "hello world"}]
        content_parts = ["Hello! How can I help you today?"]
    ...
    def test_empty_content_parts_still_estimates_completion(self):
        """Even with empty content, ``completion_tokens`` must be a number (0), not error."""
        api_messages = [{"role": "user", "content": "hello world"}]
