...
class TestFalCatalog:
    ...
    def test_image_generate_schema_does_not_reference_unavailable_edit_tool(self, image_tool):
        description = image_tool.IMAGE_GENERATE_SCHEMA["description"]
        assert "image_edit" not in description
        assert "reference_image" not in description
        assert "text-to-image" in description
