...
class TestTelegramRequestChangesButton:
    ...
    async def test_inline_keyboard_includes_request_changes_button(self):
        ...
        # InlineKeyboardMarkup wraps a list[list[InlineKeyboardButton]].
        rows = keyboard.inline_keyboard
        all_buttons = [btn for row in rows for btn in row]
        callback_data_values = [getattr(btn, "callback_data", None) for btn in all_buttons]
        assert any(
            cb is not None and cb.startswith("ea:changes:")
            for cb in callback_data_values
        ), f"Request Changes button missing from layout: {callback_data_values}"
