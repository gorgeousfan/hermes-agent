...
Phase 1 coverage (per weaviate-engram.md §8):
...
from plugins.memory.weaviate_engram import (
    SEARCH_SCHEMA,
    STORE_SCHEMA,
    FETCH_SCHEMA,
...
# ---------------------------------------------------------------------------
# Fake Engram client — replaces plugins.memory.weaviate_engram.EngramClient
# ---------------------------------------------------------------------------
...
def provider(monkeypatch, env_credentials, tmp_path):
    monkeypatch.setattr("plugins.memory.weaviate_engram.is_sdk_available", lambda: True)
    monkeypatch.setattr("plugins.memory.weaviate_engram.EngramClient", FakeEngramClient)
...
class TestIsAvailable:
    ...
    def test_false_without_sdk(self, monkeypatch, env_credentials):
        monkeypatch.setattr("plugins.memory.weaviate_engram.is_sdk_available", lambda: False)
        assert WeaviateEngramMemoryProvider().is_available() is False
