...
_SEARCH_CAPABLE_BACKENDS = {"exa", "parallel", "firecrawl", "tavily", "searxng", "brave-free", "ddgs"}
...
def _get_backend() -> str:
    ...
    # precedence, then fall back to free/local backends when no higher-priority
    # backend is configured. Firecrawl also counts as available when the managed
...
def _get_search_backend() -> str:
    ...
    """
    return _get_backend_for_capability("search")
...
def _get_extract_backend() -> str:
    ...
    3. Auto-detect from env vars
    """
    return _get_backend_for_capability("extract")
...
def _get_capability_backend(capability: str) -> str:
    ...
    if specific and _is_backend_available(specific):
        if specific == _LOCAL_OSS_BACKEND:
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == _LOCAL_OSS_BACKEND:
        return (_has_env("SEARXNG_API_URL") or _has_env("SEARXNG_URL")) and _has_env("CRAWL4AI_API_URL")
    return False
...
def _get_searxng_api_url() -> str:
    ...
    url = (os.getenv("SEARXNG_API_URL", "").strip() or os.getenv("SEARXNG_URL", "").strip()).rstrip("/")
    if not url:
            ...
            if backend == "parallel":
                ...
                return json.dumps({
                    ...
                    "error": f"{_label} is a search-only backend and cannot extract URL content. "
                             "Set web.extract_backend to crawl4ai, firecrawl, tavily, exa, or parallel.",
                }, ensure_ascii=False)
        ...
        if backend in ("searxng", "brave-free", "ddgs"):
            ...
            return json.dumps({
                "error": f"{_label} is a search-only backend and cannot crawl URLs. "
                         "Set web.crawl_backend to crawl4ai or firecrawl, or use web_search instead.",
                "success": False,
            }, ensure_ascii=False)
...
if __name__ == "__main__":
    ...
    if web_available:
        ...
        if primary_backend == "exa":
            ...
            print(f"   Using SearXNG (search only): {(os.getenv('SEARXNG_API_URL') or os.getenv('SEARXNG_URL') or '').strip()}")
        elif primary_backend == "brave-free":
            print("   Using Brave Search free tier (search only)")
        elif primary_backend == "ddgs":
