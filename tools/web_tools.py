...
def _parallel_search(query: str, limit: int = 5) -> dict:
    ...
    requested_mode = os.getenv("PARALLEL_SEARCH_MODE", "agentic").lower().strip()
    if requested_mode not in _PARALLEL_ALLOWED_SEARCH_MODES:
        requested_mode = "agentic"

    client = _get_parallel_client()
    max_results = min(limit, 20)
    if hasattr(client, "search"):
        api_mode = _PARALLEL_LEGACY_TO_GA_MODE.get(requested_mode, requested_mode)
        logger.info("Parallel search: '%s' (mode=%s, limit=%d)", query, api_mode, limit)
        ...
        api_mode = _PARALLEL_GA_TO_LEGACY_MODE.get(requested_mode, requested_mode)
        logger.info("Parallel beta search: '%s' (mode=%s, limit=%d)", query, api_mode, limit)
