...
def _build_top_level_description() -> str:
    ...
    return (
        ...
        f"PER-TURN LIMIT: You may emit at most {max_children} separate "
        f"delegate_task tool_calls in a single assistant turn (same "
        f"max_concurrent_children={max_children} cap as the 'tasks' batch). "
        f"Any extra single-task delegate_task calls in the same turn are "
        f"truncated by Hermes before execution; you will receive a guardrail "
        f"note appended to your prior assistant message describing the "
        f"truncation. To run more than {max_children} subagents in parallel, "
        f"prefer the 'tasks' batch parameter over multiple single-task "
        f"delegate_task calls.\n\n"
