...
def call_llm(
    ...
    *,
    ...
    try:
        ...
        #
        # ── Vision capability fallback ───────────────────────────────
        # When the auxiliary "vision" task targets a model the provider
        # cannot service (no image_in capability), try the strict vision
        # backend chain before giving up — even on explicit-provider setups.
        ...
        if should_fallback and (is_auto or is_capacity_error or vision_capability_error):
            ...
            if not vision_capability_error and fb_client is None:
                if is_auto:
...
async def async_call_llm(
    ...
    *,
    ...
    try:
        ...
        # ── Payment / connection / rate-limit / vision-capability fallback (mirrors sync call_llm) ──
        vision_capability_error = task == "vision" and _is_vision_capability_error(first_err)
        should_fallback = (
            _is_payment_error(first_err)
            or _is_connection_error(first_err)
